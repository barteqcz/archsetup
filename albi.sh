...
pacman -Sy btrfs-progs dosfstools inetutils xfsprogs base-devel bash-completion bluez bluez-utils nano git grub ntfs-3g sshfs networkmanager wget exfat-utils usbutils xdg-utils xdg-user-dirs unzip unrar zip p7zip os-prober plymouth --noconfirm
...
if [[ "$de" == "gnome" ]]; then
    ...
    pacman -S xorg wayland --noconfirm
    pacman -S sddm plasma kwalletmanager kate konsole dolphin spectacle ark noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra gvfs power-profiles-daemon maliit-keyboard ufw --noconfirm
